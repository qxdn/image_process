...
%% 读取图像
%img = imread('作业4图像3.tif');
img = imread('作业4图像4.tif');
figure(1);
...
% 二值化图像
%img_bw = imbinarize(J);
img_bw1 = imbinarize(img(1:1040,1:696));%对图像左半部分进行二值化
img_bw2 = imbinarize(img(1:1040,697:1392));%对图像右半部分进行二值化
img_bw = [img_bw1,img_bw2];%将二值化图像合成
